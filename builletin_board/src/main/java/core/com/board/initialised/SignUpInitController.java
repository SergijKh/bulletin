...
public class SignUpInitController {
	private static final Logger logger1 = Logger
			.getLogger(SignUpInitController.class);
	
	@Autowired
	LogService serLog;

	/**
	 * add in base user and login
	 * if  parameter login,password, email  don't empty and not  null create cookie
	 * and create session attribute  Login  
	 * @param httpRequest
	 *            HttpServletRequest
	 */
	public void addBaseLogin(HttpServletRequest httpRequest,
		...
		if (!((login.equals("")) && (!(password.equals("")) &&(!(nameUSer.equals("")))))) {
...
/**
 * 
 * method equals initialized parameter name login  and password with data base if true
 * create cookie and session attribute login (Login object)
 ...
 */
	public void initialized(HttpServletRequest httpRequest,
		...
		if (!(login.equals("")) && (!(password.equals("")))) {
			...
			logger1.info("login name ??????????" + log == null);
			if (log != null) {
				logger1.info("login name ??????????" + log.getLogin());
				if ((login.equals(log.getLogin()) && (password.equals(log
						.getPassword())))) {
					Cookie CLogin = new Cookie("login", login);
					Cookie CPassword = new Cookie("password", password);
					Cookie CIdLogin = new Cookie("idLogin", ""+log.getIdLogin());
					Cookie CnameUser = new Cookie("nameUser", log.getNameUser());
					response.addCookie(CLogin);
					response.addCookie(CPassword);
					response.addCookie(CIdLogin);
					response.addCookie(CnameUser);
					logger1.info("log id //////////"+log.getIdLogin());
					session.setAttribute("login", log);
				}
